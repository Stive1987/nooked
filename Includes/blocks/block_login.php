...
function affich_block_login($blok){
    global $user, $nuked, $bgcolor3, $bgcolor1, $db ;
	...
	if($login != 'off'){
		if (!$user){
			...
			if ($avatar != 'off'){
				$sql_avatar = mysqli_query($db,'SELECT avatar FROM ' . USER_TABLE . ' WHERE id = \'' . $user[0] . '\' ');
				list($avatar_url) = mysqli_fetch_array($sql_avatar);
    ...
    if($messpv != 'off' && $user[0] != ''){
		...
		$sql2 = mysqli_query($db,'SELECT mid FROM ' . USERBOX_TABLE . ' WHERE user_for = \'' . $user[0] . '\' AND status = 1');
		$nb_mess_lu = mysqli_num_rows($sql2);
	...
	if ($members != 'off'){
    	...
    	$sql_users = mysqli_query($db,'SELECT id FROM ' . USER_TABLE . ' WHERE niveau < 3');
    	$nb_users = mysqli_num_rows($sql_users);

    	$sql_admin = mysqli_query($db,'SELECT id FROM ' . USER_TABLE . ' WHERE niveau > 2');
    	$nb_admin = mysqli_num_rows($sql_admin);

    	$sql_lastmember = mysqli_query($db,'SELECT pseudo FROM ' . USER_TABLE . ' ORDER BY date DESC LIMIT 0, 1');
    	list($lastmember) = mysqli_fetch_array($sql_lastmember);
	...
	if ($online != 'off'){
    	...
    	if ($nb[1] > 0){
			$sql4 = mysqli_query($db,'SELECT username FROM ' . NBCONNECTE_TABLE . ' WHERE type BETWEEN 1 AND 2 ORDER BY date');
			while (list($nom) = mysqli_fetch_array($sql4)){
		...
		if ($nb[2] > 0){
			$sql5 = mysqli_query($db,'SELECT username FROM ' . NBCONNECTE_TABLE . ' WHERE type > 2 ORDER BY date');
			while (list($name) = mysqli_fetch_array($sql5)){
				   $admin_online .= '&nbsp;<b><big>·</big></b>&nbsp;<b>' . $name . '</b><br />';
			}
	
			$admin_list = '&nbsp;[<a href="#" onmouseover="AffBulle(\'&nbsp;&nbsp;' . _WHOISONLINE . '\', \'' . htmlentities(mysqli_real_escape_string($admin_online), ENT_NOQUOTES) . '\', 150)" onmouseout="HideBulle()">' . _LIST . '</a>]';
		}
		else{
			$admin_list = '';
		}
...
function edit_block_login($bid){
    global $nuked, $language, $db;

    $sql = mysqli_query($db,'SELECT active, position, titre, module, content, type, nivo, page FROM ' . BLOCK_TABLE . ' WHERE bid = \'' . $bid . '\' ');
    list($active, $position, $titre, $modul, $content, $type, $nivo, $pages) = mysqli_fetch_array($sql);
