...
function affich_block_survey($blok){
    ...
    if ($file != 'Survey'){
        ...
        $sql = mysqli_query($db,'SELECT sid, titre FROM ' . SURVEY_TABLE . ' ' . $where);
        list($poll_id, $titre) = mysqli_fetch_array($sql);
        ...
        $sql2 = mysqli_query($db,'SELECT voteID, optionText FROM ' . SURVEY_DATA_TABLE . ' WHERE sid = \'' . $poll_id . '\' ORDER BY voteID ASC');
        while (list($voteid, $optiontext) = mysqli_fetch_array($sql2)){
...
function edit_block_survey($bid){
    ...
    $sql2 = mysqli_query($db,'SELECT sid, titre FROM ' . SURVEY_TABLE . ' ORDER BY sid DESC');
    while (list($survey_id, $survey_title) = mysqli_fetch_array($sql2)){
