...
$Sql = mysql_query("SELECT N.id, N.titre, N.date, N.auteur, N.auteur_id, N.published, N.niveau, U.pseudo FROM " . NEWS_TABLE . " N, " . USER_TABLE . " U WHERE '$day' >= N.date AND U.id = N.auteur_id ORDER BY date DESC LIMIT 0, 5");
while ($row = mysql_fetch_assoc($Sql)) {
	
	if ($row['published'] == 1 && $visiteur >= $row['niveau']) {
