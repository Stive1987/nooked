...
if ($visiteur >= $level_access && $level_access > -1) {
	...
	function index() {
		global $nuked, $language, $theme, $visiteur, $db;
        // CONNECT TO DB.		
        connect();
		...
		if ($_REQUEST['op'] == 'categorie') {
			$where = "WHERE cat = '{$_REQUEST['cat_id']}' AND $day >= date AND published = '1' ";
		} elseif ($_REQUEST['op'] == 'suite' || $_REQUEST['op'] == 'index_comment') {
			$where = "WHERE id = '{$_REQUEST['news_id']}' AND $day >= date AND published = '1' ";
		} else {
			$where = "WHERE $day >= date AND published = '1' ";
		}

		$sql_nbnews = mysqli_query($db, "SELECT id FROM ".NEWS_TABLE." $where");
		$nb_news = mysqli_num_rows($sql_nbnews);

		if(!$_REQUEST['p']) $_REQUEST['p'] = 1;
		$start = $_REQUEST['p'] * $max_news - $max_news;

		if ($_REQUEST['op'] == 'categorie') {
			$WhereNews = "WHERE cat = '{$_REQUEST['cat_id']}' AND $day >= date AND published = '1' ORDER BY date DESC LIMIT $start, $max_news";
		} elseif ($_REQUEST['op'] == 'suite' || $_REQUEST['op'] == 'index_comment') {
			$WhereNews = "WHERE id = '{$_REQUEST['news_id']}' AND published = '1' ";
		} else {
			$WhereNews = "WHERE $day >= date AND published = '1' ORDER BY date DESC LIMIT $start, $max_news";
		}
		
		$sql = mysqli_query($db, "SELECT id, auteur, auteur_id, date, titre, texte, suite, cat, published, niveau, allow_comments FROM ".NEWS_TABLE." $WhereNews");
		
		if (mysqli_num_rows($sql) <= 0) {
			echo '<p style="text-align: center">' . _NONEWSINDB . '</p>';
		}
		
		while ($TabNews = mysqli_fetch_assoc($sql)) {
			$TabNews['titre'] = printSecuTags($TabNews['titre']);

			$sql2 = mysqli_query($db, "SELECT im_id FROM ".COMMENT_TABLE." WHERE im_id = '{$TabNews['id']}' AND module = 'news'");
			$nb_comment = mysqli_num_rows($sql2);

			$sql3 = mysqli_query($db, "SELECT titre, image FROM ".NEWS_CAT_TABLE." WHERE nid = '{$TabNews['cat']}'");
			$TabCat = mysqli_fetch_assoc($sql3);

			if (!empty($autor_id)) {
				$sql4 = mysqli_query($db, "SELECT pseudo FROM ".USER_TABLE." WHERE id = '{$TabNews['auteur_id']}'");
				$test = mysqli_num_rows($sql4);
			}

			if (!empty($autor_id) && $test > 0) list($auteur) = mysqli_fetch_array($sql4);
	...
	function index_comment($news_id) {
		...
		$sql2 = mysqli_query($db, "SELECT published, niveau, allow_comments FROM ".$nuked['prefix']."_news WHERE id = '".$news_id."' ");
		$TabNews = mysqli_fetch_array($sql2);
		
		if($TabNews['published'] == 1 && $visiteur >= $TabNews['niveau']) {
			
			index();
		
			$sql = mysqli_query($db, "SELECT active FROM ".$nuked['prefix']."_comment_mod WHERE module = 'news'");
	...
	function error_news() {
		...
		opentable();
		echo '<div style="text-align: center; padding: 10px;">' . _ERRORNEWS . '</div>';
		closetable();
	}
