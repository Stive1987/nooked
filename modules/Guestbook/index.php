...
if ($visiteur >= $level_access && $level_access > -1)
{
    ...
    function post_book()
    {
        ...
        function verifchamps(){
            if (document.getElementById(\'guest_name\').value.length == 0){
                alert(\'' . addslashes(_NONICK) . '\');
                return false;
            }
            if (document.getElementById(\'guest_mail\').value.indexOf(\'@\') == -1){
                alert(\'' . addslashes(_ERRORMAIL) . '\');
                return false;
            }
		...
		echo '<form method="post" action="index.php?file=Guestbook&amp;op=send_book" onsubmit="return verifchamps()">';
		echo "<table style=\"margin: auto; width: 98%; text-align: left;\" cellspacing=\"0\" cellpadding=\"2\"border=\"0\"n\>"
		...
		. "<tr><td colspan=\"2\"><textarea id=\"e_basic\" name=\"comment\" cols=\"65\" rows=\"12\"></textarea></td></tr></table>\n";
		...
		echo "<table style=\"margin: auto; width: 98%; text-align: left;\" cellspacing=\"0\" cellpadding=\"2\"border=\"0\">\n";

        echo '<p style="text-align: center; clear: left"><br /><input type="submit" name="submit_captcha" class="bouton" value="' . _SEND . '" /></p></form><br /></div>';
    ...
    function send_book($name, $email, $url, $comment)
    {
        ...
        if ($captcha == 1) Validrecaptchacode();
        echo '</div>',"\n";
