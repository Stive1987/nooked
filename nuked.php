...
while ($row = mysqli_fetch_array($sql_conf)) $nuked[$row['name']] = htmlentities($row['value'], ENT_NOQUOTES);
unset($sql_conf, $row);
...
function session_read($id){
    ...
    $sql = mysqli_query($db, 'SELECT session_vars FROM ' . TMPSES_TABLE . ' WHERE session_id = "' . $id . '"');
    if(mysqli_num_rows($sql) > 0){
        return ($sql === false) ? '' : mysqli_result($sql, 0);
    }
}
**///A SUPPRIMER ********************************************************************************************
...
function session_write($id, $data){
    ...
    $sql = mysqli_query($db, 'INSERT INTO ' . TMPSES_TABLE . ' (session_id, session_start, session_vars) VALUES ("' . $id . '", ' . time() . ', \'' . $data . '\')');

    if ($sql === false || mysqli_affected_rows($db) == 0) $sql = mysqli_query($db, 'UPDATE ' . TMPSES_TABLE . ' SET session_vars = \'' . $data . '\' WHERE session_id = "' . $id . '"');
...
function connect(){
    ...
    $db = mysqli_connect($global['db_host'], $global['db_user'], $global['db_pass'], $global['db_name']);
...
function connect() {
    ...
    if (! $layerLoaded) {
        if (! is_file($dbLayer = dirname(__FILE__) .'/Includes/nkDB.php'))
            exit(ERROR_DB_LAYER);
...
if (ini_get('session.save_handler') == 'files')
    session_set_save_handler('session_open', 'session_close', 'session_read', 'session_write', 'session_delete', 'session_db');
...
function banip() {
    ...
    $query_ban = mysqli_query($GLOBALS['nkDB']['connection'], 'SELECT `id`, `pseudo`, `date`, `dure` FROM ' . BANNED_TABLE . $where_query);
    $ban = mysqli_fetch_assoc($query_ban);

    // Si résultat positif à la recherche d'un bannissement
    if(mysqli_num_rows($query_ban) > 0) {
        // Nouvelle adresse IP
        $banned_ip = $user_ip;
    }
    // Recherche d'un cookie de banissement
        ...
        if($ip_dyn2 == $ip_dyn) {
            ...
            $query_ban2 = mysqli_query($GLOBALS['nkDB']['connection'], 'SELECT `id` FROM ' . BANNED_TABLE . ' WHERE (ip LIKE "%' . $ip_dyn2 . '%")');
...
function get_blok($side){
    ...
    $sql = mysqli_query($GLOBALS['nkDB']['connection'], 'SELECT bid, active, position, module, titre, content, type, nivo, page FROM ' . BLOCK_TABLE . ' WHERE active = ' . $active . ' ORDER BY position');
    while ($blok = mysqli_fetch_array($sql)){
...
/** A SUPPRIMER //*******************************************************************************************
    $req = mysqli_query($GLOBALS['nkDB']['connection'], "DELETE FROM " . NBCONNECTE_TABLE . " WHERE date < '" . $time."'");
**///A SUPPRIMER *********************************************************************************************
    ...
    if (isset($user_ip)){
        ...
        $req = mysqli_query($GLOBALS['nkDB']['connection'], "SELECT IP FROM " . NBCONNECTE_TABLE . " " . $where);
        $query = mysqli_num_rows($req);

        if ($query > 0){
            if (isset($user[0])){
                $req = mysqli_query($GLOBALS['nkDB']['connection'], "UPDATE " . NBCONNECTE_TABLE . " SET date = '" . $limite . "', type = '" . $user[1] . "', IP = '" . $user_ip . "', username = '" . $user[2] . "' WHERE user_id = '" . $user[0] . "'");
            }
            else{
                $req = mysqli_query($GLOBALS['nkDB']['connection'], "UPDATE " . NBCONNECTE_TABLE . " SET date = '" . $limite . "', type = '" . $user[1] . "', user_id = '" . $user[0] . "', username = '" . $user[2] . "' WHERE IP = '" . $user_ip . "'");
            }
        }
        else{
            $del = mysqli_query($GLOBALS['nkDB']['connection'], "DELETE FROM " . NBCONNECTE_TABLE . " WHERE IP = '" . $user_ip . "'");
            $req = mysqli_query($GLOBALS['nkDB']['connection'], "INSERT INTO " . NBCONNECTE_TABLE . " ( `IP` , `type` , `date` , `user_id` , `username` ) VALUES ( '" . $user_ip . "' , '" . $user[1] . "' , '" . $limite . "' , '" . $user[0] . "' , '" . $user[2] . "' )");
        }
    }

    $res = mysqli_query($GLOBALS['nkDB']['connection'], "SELECT type FROM " . NBCONNECTE_TABLE . " WHERE type = 0");
    $count[0] = mysqli_num_rows($res);
    $res = mysqli_query($GLOBALS['nkDB']['connection'], "SELECT type FROM " . NBCONNECTE_TABLE . " WHERE type BETWEEN 1 AND 2");
    $count[1] = mysqli_num_rows($res);
    $res = mysqli_query($GLOBALS['nkDB']['connection'], "SELECT type FROM " . NBCONNECTE_TABLE . " WHERE type > 2");
    $count[2] = mysqli_num_rows($res);
...
function nivo_mod($mod){
    ...
    $sql = mysqli_query($GLOBALS['nkDB']['connection'], "SELECT niveau FROM " . MODULES_TABLE . " WHERE nom = '" . $mod . "'");
    if (mysqli_num_rows($sql) == 0){
        return false;
    }
    else{
        list($niveau) = mysqli_fetch_array($sql);
        return $niveau;
    }
}
...
function admin_mod($mod){
    ...
    $sql = mysqli_query($GLOBALS['nkDB']['connection'], "SELECT admin FROM " . MODULES_TABLE . " WHERE nom = '" . $mod . "'");
    list($admin) = mysqli_fetch_array($sql);
    return $admin;
}
...
function compteur($file){
    ...
    $upd = mysqli_query($GLOBALS['nkDB']['connection'], 'UPDATE ' . STATS_TABLE . ' SET count = count + 1 WHERE type = "pages" AND nom = "' . $_GET['file'] . '"');
}
...
function initializeControlDB($prefixDB) {
    ...
    if (!isset($prefixDB)) {
        ...
        $result = mysqli_query($GLOBALS['nkDB']['connection'], 'SELECT name, value FROM ' . $prefixDB . '_config');
        if ($result == false) {
            exit(DBPREFIX_ERROR);
        } else {
            return $result;
